    ...
    res.on("finish", async () => {
      try {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          if (req.body) {
            logEntry.entityName = req.body.name || req.body.title;
            logEntry.changes = {
...
export const createActivityLog = async ({
  ...
  try {
    const logEntry = await ActivityLog.create({
      ...
      entityName,
      changes,
      description: description || generateDescription(action, entity, { entityName }),
      requiresApproval,
      status: requiresApproval ? "PENDING" : "AUTO_APPROVED",
      ipAddress,
      userAgent,
...
export const bypassApprovalForSuperAdmin = (req, res, next) => {
  // If user is super admin, their actions don't need approval
  if (req.user?.role === "SUPER_ADMIN" || req.user?.role === "superadmin") {
